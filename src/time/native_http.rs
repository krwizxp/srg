...
mod platform {
    ...
    impl Client {
        pub(super) fn fetch_head(&self, url: &str, context: &str) -> Result<HeadResponse> {
            ...
            setopt_callback(handle.0, CURLOPT_WRITEFUNCTION, write_vec_callback, context)?;
            setopt_callback(
                handle.0,
            ...
            setopt_str(handle.0, CURLOPT_URL, url_c.as_ptr(), context)?;
            setopt_str(handle.0, CURLOPT_USERAGENT, user_agent.as_ptr(), context)?;
            setopt_ptr(
                handle.0,
                CURLOPT_ERRORBUFFER,
            ...
            setopt_long(handle.0, CURLOPT_TIMEOUT, tcp_timeout_secs(), context)?;
            setopt_long(handle.0, CURLOPT_NOSIGNAL, 1, context)?;
            setopt_long(handle.0, CURLOPT_NOBODY, 1, context)?;
            setopt_long(handle.0, CURLOPT_FOLLOWLOCATION, 1, context)?;
            ...
            if perform_code != CURLE_OK {
                let perform_error = if error_buffer.first().copied().unwrap_or_default() != 0 {
    ...
    unsafe extern "C" fn write_vec_callback(
        ...
        let target = unsafe { &mut *userdata.cast::<CurlBuffer>() };
...
mod platform {
    ...
    impl Client {
        pub(super) fn fetch_head(&self, url: &str, context: &str) -> Result<HeadResponse> {
            ...
            let raw_connect =
                unsafe { WinHttpConnect(session.0, host_wide.as_ptr(), target.port, 0) };
            let connect = self.non_null_handle(raw_connect, "WinHttpConnect", context)?;
            let flags = if target.secure {
                WINHTTP_FLAG_SECURE
            } else {
                0
            };
            // SAFETY: method_wide and path_wide are NUL-terminated and connect.0 is valid.
            ...
            if target.secure {
                match self.set_ignore_revocation_offline(request.0) {
                    Ok(()) | Err(_) => {}
                }
            }
            let request_start = Instant::now();
            // SAFETY: request.0 is valid and no additional request body or headers are needed for HEAD.
            let sent = unsafe { WinHttpSendRequest(request.0, null(), 0, null(), 0, 0, 0) };
            if sent == 0_i32 {
            ...
            let response_received_inst = Instant::now();
            let date_header = self.query_date_header(request.0, context)?;
            let rtt = response_received_inst
                .checked_duration_since(request_start)
                .unwrap_or(MIN_TRANSFER_TIME)
                .max(MIN_TRANSFER_TIME);
        ...
        fn query_date_header(&self, request: HInternet, context: &str) -> Result<String> {
            let mut bytes = 0_u32;
            let mut index = 0_u32;
        ...
        fn set_ignore_revocation_offline(&self, request: HInternet) -> Result<()> {
    ...
    fn timeout_millis() -> i32 {
        i32::try_from(TCP_TIMEOUT.as_millis()).unwrap_or(5_000)
    }
