use super::{
    NetworkContext, ParsedServer, Result, TCP_TIMEOUT, TimeError, TimeSample, UrlScheme,
    http_date::parse_http_date_to_systemtime,
};
use core::{net::SocketAddr, result::Result as CoreResult, str, time::Duration};
use std::{
...
impl TcpResponseReader<'_, '_> {
        ...
        loop {
            ...
            let maybe_date_str =
                find_header_value(self.line_buffer, DATE_HEADER_PREFIX).map_err(|source| {
                    TcpAttemptError::Terminal(TimeError::parse_with_source(
    ...
    fn read_line(&mut self) -> CoreResult<usize, TcpAttemptError> {
        ...
        loop {
            ...
            let next_line_len = self
                ...
                .ok_or_else(|| {
                    TcpAttemptError::Terminal(TimeError::parse("TCP HTTP 헤더 line 길이 계산 실패"))
                })?;
            ...
            self.line_buffer
                .extend_from_slice(available.get(..chunk_len).ok_or_else(|| {
                    TcpAttemptError::Terminal(TimeError::parse("TCP HTTP 헤더 line 범위 계산 실패"))
                })?);
        ...
        self.header_bytes_seen =
            self.header_bytes_seen
                ...
                .ok_or_else(|| {
                    TcpAttemptError::Terminal(TimeError::parse("TCP HTTP 헤더 길이 계산 실패"))
                })?;
    ...
    fn read_response(&mut self) -> CoreResult<(SystemTime, Instant, bool), TcpAttemptError> {
        ...
        loop {
            ...
            let reusable = header_block.reusable(http_version);
            if let Some(age_result) = header_block.age_result {
                age_result.map_err(TcpAttemptError::Terminal)?;
            }
            let Some(final_date_sample) = header_block.date_sample else {
                return Err(TcpAttemptError::Terminal(missing_tcp_date()));
            };
            let (server_time, response_received_inst) =
                final_date_sample.map_err(TcpAttemptError::Terminal)?;
            return Ok((server_time, response_received_inst, reusable));
        }
    }
    fn read_status_line(&mut self) -> CoreResult<(u16, HttpVersion), TcpAttemptError> {
        ...
        let code = u16::from(hundreds_digit)
            .wrapping_mul(100)
            .wrapping_add(u16::from(tens_digit).wrapping_mul(10))
            .wrapping_add(u16::from(ones_digit));
        Ok((code, version))
    }
...
pub(super) fn validate_http_age_value(value: &str) -> CoreResult<(), &'static str> {
    ...
    if trimmed_value.bytes().any(|byte| !byte.is_ascii_digit()) {
        return Err("Age 헤더 값이 숫자가 아닙니다.");
    }
    if trimmed_value.bytes().any(|byte| byte != b'0') {
...
pub(super) fn fetch_server_time_sample(net_ctx: &mut NetworkContext) -> Result<TimeSample> {
    ...
    if matches!(parsed_address.scheme, UrlScheme::Https) {
        return net_ctx.native_http.fetch_head(parsed_address, "HTTPS");
    }
    let host_header = parsed_address.request_target.as_bytes();
    ...
    let connection = if let Some(connection) = cached_tcp_connection.take() {
        match sample_from_tcp_connection(
            ...
            Err(TcpAttemptError::Retryable(_)) => {
                connect_tcp_connection(parsed_address, cached_tcp_socket_addr)?
            }
            Err(TcpAttemptError::Terminal(error)) => return Err(error),
        }
    } else {
        connect_tcp_connection(parsed_address, cached_tcp_socket_addr)?
    };
    ...
    .map_err(|attempt_error| match attempt_error {
        TcpAttemptError::Retryable(error) | TcpAttemptError::Terminal(error) => error,
    })
}
fn connect_tcp_connection(
    parsed_address: &ParsedServer,
    cached_tcp_socket_addr: &mut Option<SocketAddr>,
    ...
    let stream = 'connect: {
        ...
        if let Some(cached_socket_addr) = *cached_tcp_socket_addr {
        ...
        for socket_addr in addrs {
            match TcpStream::connect_timeout(&socket_addr, TCP_TIMEOUT) {
                Ok(stream) => {
                    *cached_tcp_socket_addr = Some(socket_addr);
                    break 'connect stream;
                }
                Err(source) => {
...
fn sample_from_tcp_connection(
    ...
    reusable_connection: &mut Option<BufReader<TcpStream>>,
    ...
    let request_len = TCP_HEAD_REQUEST_PREFIX
        ...
        .ok_or_else(|| {
            TcpAttemptError::Terminal(TimeError::parse("TCP HTTP 요청 길이 계산 실패"))
        })?;
    ...
    tcp_request_buffer.extend_from_slice(host_header);
    ...
    let response_deadline = request_start_inst.checked_add(TCP_TIMEOUT).ok_or_else(|| {
        TcpAttemptError::Terminal(TimeError::parse("TCP HTTP 응답 기한 계산 실패"))
    })?;
    IoWrite::write_all(connection.get_mut(), tcp_request_buffer)
        .map_err(|source| TcpAttemptError::Retryable(TimeError::from(source)))?;
    let (server_time, response_received_inst, reusable) = TcpResponseReader {
    ...
    .read_response()?;
    let rtt_for_sample = response_received_inst.saturating_duration_since(request_start_inst);
    if reusable {
