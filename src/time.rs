use self::{
    ...
    util::{blend_rtt, parse_u32_digits},
};
cfg_select! {
    target_os = "windows" => {
...
impl SampleWorker {
    fn fetch(&mut self) -> Result<Option<u64>> {
        ...
        let request = SampleWorkerRequest { generation };
        match self.command_sender.try_send(request) {
            Ok(()) => {
...
struct AppState<'worker> {
    ...
    pending_sample_request: Option<(u64, SampleRequestKind)>,
...
impl NetworkContext {
    fn new(host: Arc<ParsedServer>) -> Self {
        Self {
            host,
...
impl AppState<'_> {
    ...
    fn begin_final_countdown_sampling(&mut self, initial_sample_interval: Duration) -> Result<()> {
        if self.final_countdown_sampler.is_none() {
            ...
            let join_handle = thread::Builder::new()
                ...
                .spawn(move || {
                    let mut network_context = NetworkContext::new(host);
    ...
    fn ensure_sample_request(&mut self, kind: SampleRequestKind) -> Result<()> {
        ...
        match self.sample_worker.fetch() {
            Ok(request) => {
                self.pending_sample_request = request.map(|generation| (generation, kind));
                Ok(())
            }
            Err(fetch_err) => {
    ...
    fn handle_calibrate_on_tick<'message>(
        ...
        let sample_result = match self.poll_sample_request() {
            Ok(Some((SampleRequestKind::Calibration, result))) => result,
            Ok(None | Some((SampleRequestKind::Baseline { .. }, _))) => {
                ...
                return ActivityTransition::stay(Activity::CalibrateOnTick);
            }
            Err(worker_err) => {
        ...
        ActivityTransition::stay(Activity::CalibrateOnTick)
    }
    fn handle_final_countdown<'message>(
        ...
        let now = Instant::now();
        let current_server_time = server_time.current_server_time_at(now);
        let (latest_sample, sampler_finished, poisoned) =
            if let Some(active) = self.final_countdown_sampler.as_mut() {
                let sampler_finished = active.join_handle.is_finished();
                match active.sample_slot.try_lock() {
                    Ok(mut slot) => (slot.take(), sampler_finished, false),
                    Err(TryLockError::WouldBlock) => (None, sampler_finished, false),
                    Err(TryLockError::Poisoned(_)) => (None, sampler_finished, true),
                }
            } else {
                (None, false, false)
            };
        ...
        let sample_error_reported = match latest_sample {
            Some(Ok(sample)) => {
            ...
            None if poisoned => self.append_final_countdown_sample_error(
                now,
                msg_buf,
                TimeError::parse("카운트다운 샘플 상태 잠금 실패"),
            ),
            None if sampler_finished => self.append_final_countdown_sample_error(
                now,
                msg_buf,
        ...
        ActivityTransition::stay(Activity::FinalCountdown {
            server_time,
            target_time,
        })
    }
    fn handle_final_countdown_sample<'message>(
        ...
        let live_rtt = blend_rtt::<7>(old_rtt, sample_rtt);
    ...
    fn handle_measure_baseline_rtt<'message>(
        ...
        if self.baseline_rtt_attempts == 0 && self.pending_sample_request.is_none() {
        ...
        let sample_result = match self.poll_sample_request() {
            Ok(Some((SampleRequestKind::Baseline { attempt_index }, result))) => {
                (attempt_index, result)
            }
            Ok(None | Some((SampleRequestKind::Calibration, _))) => {
                if had_pending_request || now < self.baseline_rtt_next_sample_at {
                    return ActivityTransition::stay(Activity::MeasureBaselineRtt);
                }
        ...
        ActivityTransition::message(Activity::CalibrateOnTick, msg_buf)
    }
        ...
        match *activity {
            ...
            Activity::Retrying { started_at } => {
                if now.saturating_duration_since(started_at) >= RETRY_DELAY {
                    ...
                    ActivityTransition::stay(Activity::Retrying { started_at })
                }
            }
        }
    }
    fn poll_sample_request(&mut self) -> Result<Option<(SampleRequestKind, Result<TimeSample>)>> {
        let Some((pending_generation, pending_kind)) = self.pending_sample_request else {
            loop {
            ...
            match self.sample_worker.response_receiver.try_recv() {
                Ok(response) => {
                    if response.generation == pending_generation {
                        self.pending_sample_request = None;
                        return Ok(Some((pending_kind, response.result)));
                    }
                }
...
fn spawn_sample_worker(initial_host: Arc<ParsedServer>) -> Result<SampleWorker> {
    ...
    let join_handle = thread::Builder::new()
        ...
        .spawn(move || {
            let mut network_context = NetworkContext::new(initial_host);
            while let Ok(SampleWorkerRequest { generation }) = command_receiver.recv() {
                let result = fetch_server_time_sample(&mut network_context);
                if response_sender
                    .send(SampleWorkerResponse { generation, result })
                    .is_err()
                {
                    return;
                }
