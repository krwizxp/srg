...
pub(super) fn checked_add_one_u64(value: u64, err_msg: &'static str) -> Result<u64> {
    let Some(next_value) = value.checked_add(ONE_BASED_OFFSET_U64) else {
        return Err(err_msg.into());
    };
    Ok(next_value)
}
pub(super) const fn checked_add_one_u8(value: u8) -> Option<u8> {
    value.checked_add(ONE_BASED_OFFSET_U8)
}
pub(super) const fn checked_add_one_usize(value: usize) -> Option<usize> {
    value.checked_add(ONE_BASED_OFFSET_USIZE)
}
pub(super) fn glyph_from_low_nibble(value: u64) -> Option<char> {
    GLYPHS
        .get(usize::from(low_u8_from_u64(value & NIBBLE_MASK_U64)))
        .copied()
}
pub(super) fn galaxy_coord<const SUB: u16, const ADD: u16>(value: u16) -> Result<u16> {
