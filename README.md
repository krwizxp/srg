...
- 서버 시간 정밀 동기화(`time.rs`)
  - TCP 또는 native HTTP로 HTTP Date 헤더 수집
  - RTT 기반 보정
...
### 참고 사항
- 역지오코딩은 외부 네트워크(예: OSM Nominatim) 연결과 HTTP(S) 출처가 필요합니다.
- 클립보드 복사는 브라우저/보안 컨텍스트 정책(HTTPS 등)에 맞춰 동작합니다.
## 요구사항
- Rust 1.96 이상
## 빌드 및 실행
```bash
...
- 캐시: 매 실행마다 새 빌드 사용
- 기본 Artifact 이름: `srg-result-console-log`, 필요 시 `srg-result-random-data`
- 기본 포함 파일: `srg-result-console.log`, 필요 시 `srg-result-random_data.txt`
실행 방법:
...
입력값:
- `action`: 실행할 기능 선택
- `count`: `generate-multiple`에서 사용할 생성 개수(1~100000)
- `players`, `results`: `ladder`에서 사용할 한 줄 쉼표 구분 입력값(2~512개, 빈 항목 불가, 개수 일치 필요)
- `int_min`, `int_max`: `random-integer` 입력 범위(64비트 정수)
...
- `artifact_name`: 업로드할 Artifact/파일 이름 prefix. 제어문자/파일명 금지 문자는 `-`로 바꾸고 앞 80자만 사용합니다. Artifact 이름은 `<artifact_name>-console-log`/`<artifact_name>-random-data`, 포함 파일은 `<artifact_name>-console.log`/`<artifact_name>-random_data.txt` 형식입니다.
실행 참고:
- GitHub Actions 수동 실행 UI에서는 입력값 기반 실행을 사용합니다.
- `generate-single`/`generate-multiple` 결과는 runner에서 생성된 `random_data.txt`를 `<artifact_name>-random_data.txt` 파일로 복사하고 `<artifact_name>-random-data` Artifact로 업로드합니다.
...
메뉴 7은 직접 입력한 `num_64`/`supp` 값을 SRG 출력 형식으로 변환·검증하는 기능이며, 새 하드웨어 난수값을 가져오지 않습니다.
RDSEED 지원 환경에서는 RDSEED를 우선 사용하고, RDSEED 응답을 5분 동안 받지 못하면 RDRAND로 전환합니다.
RDSEED/RDRAND가 동일한 64비트 값을 비정상적으로 반복 반환하면 하드웨어 RNG 이상으로 보고 중단합니다.
## 출력 파일
- 기본 저장 파일: `random_data.txt`
- 대량 생성 출력 순서는 작업 스레드의 생성 완료 순서이며, 각 행의 순번 자체에는 의미를 두지 않습니다.
## 서버 시간 동기화 관련 의존성
- Linux/macOS HTTPS 시간 조회: native libcurl 사용
- Linux/macOS native libcurl은 HTTPS protocol allowlist 설정을 위해 7.85.0 이상이 필요합니다.
- 서버 주소에서 스킴을 생략하면 HTTPS가 기본값입니다. 평문 HTTP가 필요할 때만 `http://`를 명시하세요.
- 예: `example.com`, `example.com:8443`, `[2001:db8::1]`는 HTTPS로 처리되고, `http://example.com`만 평문 HTTP로 처리됩니다.
- Linux 액션 실행: X11/XTest 런타임 라이브러리(좌클릭/F5 액션 사용 시 필요)
