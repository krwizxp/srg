...
```
## GitHub Actions 실행파일
`.github/workflows/ci.yml`은 `ubuntu-latest`, `macos-latest`, `windows-latest`에서 `cargo build --release --locked`를 수행하고 결과 실행 파일을 Artifact로 업로드합니다.
- Linux Artifact 파일: `srg-linux-x64.tar.gz`
  - 내부 실행 파일: `srg-linux-x64`
- macOS Artifact 파일: `srg-macos.tar.gz`
  - 내부 실행 파일: `srg-macos`
- Windows Artifact 파일: `srg-windows-x64.exe`
Linux/macOS는 실행 권한 보존을 위해 바이너리를 `tar.gz`로 묶어 업로드합니다. Windows는 빌드 결과를 `srg-windows-x64.exe`로 이름만 바꿔 그대로 업로드합니다.
...
- 캐시: 매 실행마다 새 빌드 사용
- 기본 Artifact 파일: `srg-result-console.log`, 필요 시 `srg-result-random_data.txt`
실행 방법:
...
- `artifact_name`: 업로드할 파일 이름 prefix (`<artifact_name>-console.log`, 필요 시 `<artifact_name>-random_data.txt`)
실행 참고:
- GitHub Actions 수동 실행 UI에서는 입력값 기반 실행을 사용합니다.
- `generate-single`/`generate-multiple` 결과는 runner에서 생성된 `random_data.txt`를 `<artifact_name>-random_data.txt` 이름으로 복사해 업로드합니다.
